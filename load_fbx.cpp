...
bool LoadFBX::GetMesh(FbxNode* pNode, MODEL& model)
{
	...
	for (int i = 0; i < childNodeNum; ++i)
	{
		...
		case FbxNodeAttribute::eMesh:
		{
			...
			//ローカル行列取得
			FbxAMatrix& mtxFbx = pChild->EvaluateLocalTransform();
			XMFLOAT4X4 mtxTransform;
			mtxTransform._11 = (float)mtxFbx.Get(0, 0);
			mtxTransform._12 = (float)mtxFbx.Get(0, 1);
			mtxTransform._13 = (float)mtxFbx.Get(0, 2);
			mtxTransform._14 = (float)mtxFbx.Get(0, 3);
			mtxTransform._21 = (float)mtxFbx.Get(1, 0);
			mtxTransform._22 = (float)mtxFbx.Get(1, 1);
			mtxTransform._23 = (float)mtxFbx.Get(1, 2);
			mtxTransform._24 = (float)mtxFbx.Get(1, 3);
			mtxTransform._31 = (float)mtxFbx.Get(2, 0);
			mtxTransform._32 = (float)mtxFbx.Get(2, 1);
			mtxTransform._33 = (float)mtxFbx.Get(2, 2);
			mtxTransform._34 = (float)mtxFbx.Get(2, 3);
			mtxTransform._41 = (float)mtxFbx.Get(3, 0);
			mtxTransform._42 = (float)mtxFbx.Get(3, 1);
			mtxTransform._43 = (float)mtxFbx.Get(3, 2);
			mtxTransform._44 = (float)mtxFbx.Get(3, 3);

			GetPosition(mesh, mtxTransform);
			GetVertexNormal(mesh, mtxTransform);
			GetVertexTangent(mesh, mtxTransform);
			GetVertexUV(mesh);
...
// 頂点座標を取得する
void LoadFBX::GetPosition(FbxMesh* mesh, XMFLOAT4X4& mtx)
{
	// メッシュマテリアルリストの取得
...
void LoadFBX::GetBone(FbxMesh* mesh)
{
	...
	if (skinMax == 0)
	{
		...
		XMMath::Identity(ofBone.initMtx);
...
// 法線の取得
void LoadFBX::GetVertexNormal(FbxMesh* mesh, XMFLOAT4X4& mtx)
{
	// 法線セット数を取得
...
// Tangentの取得
void LoadFBX::GetVertexTangent(FbxMesh* mesh, XMFLOAT4X4& mtx)
{
	// Tangentセット数を取得
...
bool LoadFBX::CreateModelData(MODEL& model)
{
	if (meshMaterialList_.size() / 3 == 1)
	{
		...
		bool isCreate = this->CreateVeretx(model, (int)index.size(), &index[0], tempUVSet_[0]);
		if (!isCreate)
		{
			return false;
		}
	...
	else
	{
		...
		for (int i = 0; i < materialMax; ++i)
		{
			...
			if (index.size() != 0)
			{
				...
				bool isCreate = this->CreateVeretx(model, (int)index.size(), &index[0], tempUVSet_[materialCount_[1]]);
				if (!isCreate)
				{
					return false;
				}
...
bool LoadFBX::CreateVeretx(MODEL& model, int indexNum, const int* index, UVSet uv)
{
	...
	{
		MESH tempMesh;		
		XMMath::Identity(tempMesh.frameTransMtx);
